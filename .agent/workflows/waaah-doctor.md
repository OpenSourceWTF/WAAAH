...
2. **Initialize State**
   - Run command:
     ```bash
     mkdir -p .waaah/doctor .waaah/health/history
     if [ ! -f .waaah/doctor/state.json ]; then
       echo '{"last_sha": "", "last_run": "", "violations": []}' > .waaah/doctor/state.json
     fi
     ```
...
3. **Repo Scan (Changes Detected)**
   ...
   - IF `[ -z "$CHANGES" ]`:
     ...
     - Proceed to Step 7 (Update State).
   - ELSE:
...
5. **Structure/Quality Analysis**
   ...
   - **Duplicate Detection**:
     ...
     for EXPORT in $EXPORTS; do
       MATCHES=$(grep -rl "export.*$EXPORT" packages/ --include="*.ts" | grep -v "$FILE" | head -3)
       if [ -n "$MATCHES" ]; then
         # Add to VIOLATIONS for each duplicate
       fi
     done
     ```
   - Proceed to Step 6.
...
6. **Generate Health Report**
   - Create report at `.waaah/health/latest.md`:
     ```markdown
     # WAAAH Health Report
     
     **Generated:** <timestamp>
     **Analyzed Commit:** <LATEST_SHA>
     **Previous Commit:** <LAST_SHA>
     ...
     - [ ] T-XXX: Fix coverage in <file>
     - [ ] T-XXX: Refactor <file> (complexity)
     
     ---
     
     *Report generated by waaah-doctor*
     ```
   - Archive to `.waaah/health/history/<timestamp>.md`
   - `update_progress({ message: "Health report generated: <violation_count> issues found", phase: "REPORTING" })`
